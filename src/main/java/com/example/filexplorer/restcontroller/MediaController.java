...
public class MediaController {
    ...
    @GetMapping("/video/{base64path}")
    public void getVideo(HttpServletRequest request, HttpServletResponse response, @PathVariable String base64path)
        ...
        String path = new String(Base64.getDecoder().decode(base64path.getBytes()));
    ...
    @GetMapping("/image/{base64path}")
    public HttpEntity<byte[]> getImage(HttpServletRequest request, HttpServletResponse response,@PathVariable String base64path)
            throws IOException {
        ...
        DataInputStream dataInputStream = new DataInputStream(new FileInputStream(imgFile));
        ...
        if(imgFile.getName().toLowerCase().endsWith("jpeg")){
            headers.setContentType(MediaType.IMAGE_JPEG);
        }
